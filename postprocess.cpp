...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "../3rdparty/rknn/model/coco_80_labels_list.txt"
                   ...
                   int32_t zp, float scale)
{
  ...
  例如  对于1x255x80x80
  |<---- 80-----> | _ _ _ _             预测框
  [0,1,2,3,....79]    ⬆                 x: 80*80*1 flaot            \
  [0,1,2,3,....79]    80                y: 80*80*1 float            |
    ...                                 w: 80*80*1 flaot            |--->  (4+1+80)*80*80 ->(PROP_BOX_SIZE * a + 4) * grid_len + i * grid_w + j;
  [0,1,2,3,....79]_ _ ⬇_ _              h: 80*80*1 float            |
                                        obj_score: 80*80*1 float    |
                                        class_score:80*80*80 flaot  /
  */
  int    validCount = 0;
  ...
  for (int a = 0; a < 3; a++) {
    for (int i = 0; i < grid_h; i++) {
      for (int j = 0; j < grid_w; j++) {
        ...
        int8_t box_confidence = input[index];
        if (box_confidence >= thres_i8) {
          ...
          int8_t maxClassProbs = in_ptr[5 * grid_len];     //  比较后80组class_score  找出置信度最大值
          int    maxClassId    = 0;                        //  将数据看成是  6400*80的矩阵，每一行代表80x80个网格单元
          for (int k = 1; k < OBJ_CLASS_NUM; ++k) {        // 则前四行是80x80个网格单元的预测框的 x,y,w,h 
            int8_t prob = in_ptr[(5 + k) * grid_len];      // 第五行是  obj_score  代表检测的目标在80种类别的概率
            if (prob > maxClassProbs) {                    // 第六行到第85行 共80行 是  class_score  代表每种类别的概率
              maxClassId    = k;
              maxClassProbs = prob;
            }
          }
